...
def compute_projection_diagnostics(
    ...
    *,
    ...
    """Compute selection-oriented diagnostics for every 2D projection."""
    ...
    for key, x_column, y_column in _projection_columns(projections):
        coordinates = projections[[x_column, y_column]].to_numpy(dtype=np.float64)
...
def _projection_columns(frame: pd.DataFrame) -> list[tuple[str, str, str]]:
    columns = set(frame.columns)
    return [
        (str(column)[:-2], str(column), f"{str(column)[:-2]}_y")
