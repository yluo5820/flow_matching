    ...
    try:
        for batch in batches:
            ...
            if objective.loss != "mse":
                raise ValueError("Deterministic probe replay supports only ordinary MSE.")
            losses.append((prediction - expected).square().flatten(1).mean(1).cpu().double())
    finally:
...
def _validate_ordinary_probe_config(config: dict[str, Any]) -> None:
    ...
    if str(objective.get("name", "flow_matching")).lower() != "flow_matching":
        raise ValueError("Deterministic probes require the ordinary flow_matching objective.")
    if objective.get("modifiers", []) or float(objective.get("straightness_weight", 0)):
        raise ValueError("Deterministic probes must not use objective modifiers.")
    capacity = (config.get("model", {}) or {}).get("capacity", {}) or {}
    if bool(capacity.get("enabled", False)):
        raise ValueError("Deterministic probes must not enable model capacity adapters.")


def _validate_ordinary_objective(objective: Any) -> None:
    if str(getattr(objective, "name", "")).lower() != "flow_matching":
        raise ValueError("Deterministic probes require a flow-matching objective.")
    if getattr(objective, "modifiers", ()):
        raise ValueError("Deterministic probes must not use objective modifiers.")
    if float(getattr(objective, "straightness_weight", 0.0)) != 0.0:
        raise ValueError("Deterministic probes must not use straightness regularization.")
    if float(getattr(objective, "interpolant_acceleration_weight", 0.0)) != 0.0:
        raise ValueError("Deterministic probes must not regularize interpolant acceleration.")
...
def _validate_capacity_disabled(model: nn.Module) -> None:
    ...
    if callable(metadata) and bool(metadata().get("enabled", False)):
        raise ValueError("Deterministic probes must use a capacity-disabled model.")
