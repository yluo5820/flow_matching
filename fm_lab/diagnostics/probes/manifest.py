...
class ProbeManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.schema_version) != _SCHEMA_VERSION:
            raise ValueError(f"Unsupported probe manifest schema version: {self.schema_version}.")
...
def build_probe_manifest(
    ...
    *,
    ...
    if batch_size < 1 or rows_per_class_per_stratum % batch_size:
        raise ValueError("rows_per_class_per_stratum must be divisible by batch_size.")
    classes = np.unique(labels)
    if len(classes) < 2:
        raise ValueError("Probe manifests require at least two classes.")
    rng = np.random.RandomState(seed)
    fields: dict[str, list[np.ndarray]] = {field_name: [] for field_name in _ARRAY_FIELDS}
    microbatch_id = 0
    max_seed = np.iinfo(np.int64).max
    for class_id in classes:
        class_ids = sample_ids[labels == class_id]
        if len(class_ids) < rows_per_class_per_stratum:
            raise ValueError(f"Class {class_id} lacks enough unique examples for each stratum.")
        for stratum_id, (low, high) in enumerate(time_strata):
            ...
            fields["labels"].append(np.full(rows_per_class_per_stratum, class_id, dtype=np.int64))
            ...
            fields["timesteps"].append(
                rng.uniform(lower, upper, size=rows_per_class_per_stratum).astype(np.float64)
            )
    ...
    return ProbeManifest(
        ...
        **{field_name: np.concatenate(chunks) for field_name, chunks in fields.items()},
    )
